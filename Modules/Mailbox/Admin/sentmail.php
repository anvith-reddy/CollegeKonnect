...
$sql = "SELECT count(*) FROM mail  WHERE sender='$user' AND sent=1" ;
...
$sql2 = " SELECT * FROM mail WHERE sender='$user' AND sent = 1 order by mdate DESC limit ". ($lower_limit)." ,  ". ($page_limit). " ";
$numrows = "0";
?>
	...
	$numrows = mysqli_num_rows($result);
				...
				?>
							...
							<th>
								...
								<button type='submit' name='archive'><i class='fa fa-star text-yellow'></i></button>
				...
				<?php
					...
					<strong>SORRY!</strong> No Mails in your Outbox.
				</div>';
					}
				?>
<tr>
...
<select name="show" onChange="changeDisplayRowCount(this.value);">
  ...
  <td valign="top" align="center" >
	...
	if ( ($pagenum-1) > 0) {
	?>	
	 <a href="javascript:void(0);" class="links" onclick="sentmail('<?php echo $page_limit;  ?>', '<?php echo 1; ?>');"><button>First</button></a>
	<a href="javascript:void(0);" class="links"  onclick="sentmail('<?php echo $page_limit;  ?>', '<?php echo $pagenum-1; ?>');"><button>Previous</button></a>
	<?php
	}
	//Show page links
	for($i=1; $i<=$last; $i++) {
...
?>
	<a href="javascript:void(0);" class="links"  onclick="sentmail('<?php echo $page_limit;  ?>', '<?php echo $i; ?>');" ><button><?php echo $i ?></button></a>
<?php 
	}
} 
if ( ($pagenum+1) <= $last) {
?>
	<a href="javascript:void(0);" onclick="sentmail('<?php echo $page_limit;  ?>', '<?php echo $pagenum+1; ?>');" class="links"><button>Next</button></a>
<?php } if ( ($pagenum) != $last) { ?>	
	<a href="javascript:void(0);" onclick="sentmail('<?php echo $page_limit;  ?>', '<?php echo $last; ?>');" class="links" ><button>Last</button></a> 
<?php
	} 
?>
</td>
